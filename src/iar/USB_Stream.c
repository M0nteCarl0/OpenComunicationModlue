...
int USB_Stream_PutData(USB_OTG_CORE_HANDLE *Dev, int Received) {
  ...
  if (ReceiveStack.Datacount > ReceiveStack.MaxRxCount) {
    DCD_SetEPStatus(Dev, EP_ID_OUT, USB_OTG_EP_RX_NAK);
    DCD_EP_Flush(Dev, EP_ID_OUT);
    Ret = 0;
  ...
  DCD_EP_PrepareRx(Dev, EP_ID_OUT, ReceiveStack.ReceiveBuffer,
                   ReceiveStack.MaxRxCount);
  return Ret;
}

int USB_Stream_GetMaxRxCount(void) { return ReceiveStack.MaxRxCount; }
...
int USB_Stream_GetStateTxReady(void) { return ReceiveStack.TxReady; }

void USB_Stream_SetStateTxReady(void) { ReceiveStack.TxReady = 1; }

void USB_Stream_ResetStateTxReady(void) { ReceiveStack.TxReady = 0; }

void USB_Stream_SetReceiveBuferSize(int BufferSize) {
  USB_Stream_USB_Stack_Init(&ReceiveStack, BufferSize);
}

int USB_Stream_GetReceiveBuferSize(void) { return ReceiveStack.MaxRxCount; }
