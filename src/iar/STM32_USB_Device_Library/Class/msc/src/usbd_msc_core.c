...
/**
  ...
  */ 
#define DATAS  32276
 	static uint8_t _DATA[DATAS];
   static uint8_t _DATAOUT[DATAS];
	static uint32_t i;
	uint8_t TST[DATAS] = {0};
           
           
		typedef  union ShortTo2Byte
{
        uint16_t SRC;
	uint8_t Dest[2];
}Convet;
...
static  Convet T;
   static uint16_t Size =  0;
	static  uint8_t  RxV =	0;
	static u32 PckSize =0;
	static u8 Send = 0;
                            ...
                            uint8_t cfgidx)
{
 ...
 // MSC_BOT_Init(pdev); 
	...
	DCD_EP_PrepareRx (pdev, MSC_OUT_EP, _DATAOUT, DATAS);
  return USBD_OK;
}
...
{
   i = USBD_GetRxCount (pdev, MSC_OUT_EP);
	
	PckSize =  ((USB_OTG_CORE_HANDLE*)pdev)->dev.in_ep[epnum].xfer_len;
	...
	if(PckSize == DATAS )
	{
	  ...
	  ((USB_OTG_CORE_HANDLE*)pdev)->dev.in_ep[epnum].xfer_len  = 0;
...
void ReadInFS( void  *pdev,uint32_t _DataCount,uint8_t* _DATA)
{
...
void WriteinFS(void  *pdev, uint32_t _DataCount,uint8_t* _DATA)
{
       DCD_EP_Tx(pdev,
           MSC_IN_EP,
                 _DATA,
...
void WordTwoBytes (uint16_t WordIn, uint8_t *Byte1, uint8_t *Byte2)
{
__disable_irq();

  *Byte1 = (uint8_t) (WordIn & 0x7F);
  *Byte2 = (uint8_t) ((WordIn >> 7) & 0x7F);
    
__enable_irq();
};
//------------------------------------------------------------------------------
void WordTwoBytesNoIRQ (uint16_t WordIn, uint8_t *Byte1, uint8_t *Byte2)
{
  *Byte1 = (uint8_t) (WordIn & 0x7F);
  *Byte2 = (uint8_t) ((WordIn >> 7) & 0x7F);
};
//------------------------------------------------------------------------------
void TwoBytesWord (uint8_t Byte1, uint8_t Byte2, uint16_t *WordOut)
{
__disable_irq();

  *WordOut  = (Byte1 & 0x7F);
  *WordOut |= ((((uint16_t) Byte2) & 0x7F) << 7);
