...
* @version V 0.5.4
* @date    5-june-2017/
* @brief   This file provides USB Device Fxs for stm32f405 dstart
*         
*/ 
...
const  uint16_t               OCM_VID              = 0x63;//»ндентификатор производител€
const  uint16_t               OCM_PID               = 0x5; //»ндентификатор устройства
char*                         OCM_StringDescriptor;//—троковой описатель(не испольщуетьс€)
int                           DataAvalible = 0;
...
uint16_t                      RxCount = 0;
uint16_t                      BytesRs; 
USBD_Class_cb_TypeDef         OCM_USBD_Class;//Ёеземпл€р устройства USB STM32
USBD_DEVICE                   OCM_USBD_DEVICE;
USBD_Usr_cb_TypeDef           OCM_USBD_Usr_cb;//Ёеземпл€р функций пользовател€ USB STM32
#if defined ( __ICCARM__ ) /*!< IAR Compiler   ¬ключение выравyивани€ по машиному слову(32 бита)*/
...
__ALIGN_BEGIN  USB_configuration_descriptor OCM_USB_configuration_descriptor __ALIGN_BEGIN ; 
...
/***************************************************************************/
  ...
  {
     USBD_GetString(OCM_InterfaceName,UnicodeStringDescriptorInterfaceName,length);
...
/***************************************************************************/
  uint8_t  OCM_Init         (void *pdev , uint8_t cfgidx)
  {
    ...
    DCD_EP_PrepareRx(pdev,EP_ID_IN,USB_Stream_GetPointerBuffer(),USB_Stream_GetMaxRxCount());
     return USBD_OK;
  }
/***************************************************************************/
...
/***************************************************************************/
  ...
  {
   uint32_t  Received_DataSize =  OCM_GetReceivedByteCountFromEndpont (pdev, EP_ID_OUT);
   USB_Stream_PutData(pdev,Received_DataSize);
   return USBD_OK;
  }
/***************************************************************************/
  uint8_t  OCM_SOF          (void *pdev)
  {
     return USBD_OK;
  }
/***************************************************************************/
  uint8_t  OCM_IsoINIncomplete  (void *pdev)
  {
     return USBD_OK;
  }
/***************************************************************************/
  uint8_t  OCM_IsoOUTIncomplete  (void *pdev)
  {
     return USBD_OK;
  }
/***************************************************************************/
...
void OCM_LowLevelInit(void)
{
  ...
  OCM_InitUSBD_DEVICE();
  OCM_InitUSBD_Usr_cb();
  USBD_Init(&OCM_Handle,USB_OTG_FS_CORE_ID,&OCM_USBD_DEVICE,&OCM_USBD_Class,&OCM_USBD_Usr_cb);
...
void OCM_InitDescriptors(void)
{
  ...
  OCM_USB_DeviceDescriptor.bMaxPacketSize0 = 64;
  ...
  OCM_USB_device_qualifier.bMaxPacketSize0 = 64;
  OCM_USB_device_qualifier.bNumConfigurations = 1;
  OCM_USB_device_qualifier.Reseverd = 1; 
...
void OCM_InitUSBD_DEVICE(void)
{
   ...
   OCM_USBD_DEVICE.GetConfigurationStrDescriptor =&OCM_GetConfigurationStrDescriptor;
   ...
   OCM_USBD_DEVICE.GetManufacturerStrDescriptor =&OCM_GetManufacturerStrDescriptor; 
...
/***************************************************************************/
 ...
 {
     USBD_OTG_ISR_Handler(&OCM_Handle);
 }
 /***************************************************************************/
 ...
 /***************************************************************************/
 uint16_t  OCM_GetReceivedByteCountFromEndpont(USB_OTG_CORE_HANDLE  *pdev , uint8_t epnum)
{
