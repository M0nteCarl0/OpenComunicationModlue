...
void LED_Init(void)
{
  ...
  GPIO_InitTypeDef GPIO_InitStructure;
                                    /* Configure USARTy Rx as input floating */
  GPIO_InitStructure.GPIO_Pin   =   LEDS.IO_PIN_LED_RED|
                                   LEDS.IO_PIN_LED_GREEN;
...
void LED_RedOn(void)
{
 // GPIO_SetBits(LEDS.IO_PORT_LEDS, LEDS.IO_PIN_LED_RED);
   LED_Set_RED(&LEDS);
 
}
void LED_RedOff(void)
{
   //GPIO_ResetBits(LEDS.IO_PORT_LEDS, LEDS.IO_PIN_LED_RED); 
     LED_Reset_RED(&LEDS);
}
